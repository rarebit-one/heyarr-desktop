...
fun rememberCover(session: AppSession, type: MediaType, title: String, nodeArtPath: String?, year: Int? = null, creator: String? = null, feedRef: String? = null): State<Cover> =
    produceState(initialValue = Cover(nodeArtPath?.let { session.artwork.peek(it) }), nodeArtPath, title, type, session.config.externalMetadata) {
        if (nodeArtPath != null) {
            val bmp = session.artwork.load(nodeArtPath)
            if (bmp != null) { value = Cover(bmp); return@produceState }
        }
