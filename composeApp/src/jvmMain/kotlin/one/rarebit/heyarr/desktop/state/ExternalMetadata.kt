 ...
 * kind; every answer (including "nothing found") is cached on disk under the XDG
 * cache dir so a title is asked about at most once a week. Look-ups are
 * single-flighted per key and rate-limited per host (MusicBrainz asks for 1 req/s and
 * a real User-Agent; we give every host both).
 *
...
class ExternalMetadata(
    ...
    companion object {
        ...
        private val client: HttpClient by lazy { HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(8)).followRedirects(HttpClient.Redirect.NORMAL).build() }

        fun httpGet(url: String): String? = runCatching {
            val req = HttpRequest.newBuilder(URI.create(url)).timeout(Duration.ofSeconds(15)).header("User-Agent", USER_AGENT).header("Accept", "application/json, application/rss+xml, application/xml, text/xml;q=0.9, */*;q=0.5").GET().build()
            val resp = client.send(req, HttpResponse.BodyHandlers.ofString())
            if (resp.statusCode() == 200) resp.body() else null
        }.getOrNull()

        /** The disabled instance for previews and tests. */
        val NONE = ExternalMetadata(enabled = { false }, fetch = { null })
    }
}
