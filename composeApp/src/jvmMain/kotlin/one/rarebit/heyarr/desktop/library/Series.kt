 ...
 * `Season 02` / `Specials`), and `GET /works/{id}/assets` inlines that label on every
 * file. The episode number and title are not on the wire — they live in the filename
 * the scanner matched (`S04E01`, `2x05`, `Episode 5 - Title`), so this reads them back
 * the same way the node did. Ported from heyarr-mobile's `library.Series`; pure and
 * JVM-tested.
 */
data class Episode(
    val asset: Track,
    ...
    /** The thumbnail's relative content path, for the artwork loader. */
...
object Series {
     ...
     */
    fun seasons(assets: List<Track>): List<Season> {
     ...
     */
    fun episode(asset: Track): Episode? {
    ...
    fun isThumbnail(asset: Track): Boolean {
    ...
    fun isSubtitle(asset: Track): Boolean {
    ...
    internal fun stemKey(asset: Track, stripThumb: Boolean, stripLang: Boolean = false): String {
     ...
     * `WEB-DL`, `x265`, a year, a bracketed tag, …).
     */
    ...
    fun qualityTags(asset: Track): List<String> {
    ...
    private fun isEpisodeFile(asset: Track): Boolean {
