...
object SessionInfoJson {
    fun parse(body: String): SessionInfo? {
        ...
        return SessionInfo(
            ...
            scopes = JsonScan.arrayOf(o, listOf("scopes"))?.let { one.rarebit.heyarr.desktop.state.RecentSearches.parseStrings(it) } ?: emptyList(),
...
object ProviderJson {
    fun list(body: String): List<ProviderInfo> = JsonScan.objectsOf(body, listOf("providers", "items")).mapNotNull { p ->
        ProviderInfo(
            ...
            capabilities = JsonScan.arrayOf(p, listOf("capabilities"))?.let { one.rarebit.heyarr.desktop.state.RecentSearches.parseStrings(it) } ?: emptyList(),
...
object CapabilitiesJson {
    fun parse(body: String): Capabilities {
        ...
        val available = JsonScan.arrayOf(root, listOf("available"))?.let { one.rarebit.heyarr.desktop.state.RecentSearches.parseStrings(it) } ?: emptyList()
